...
def plot_chords_vs_time(record):
    binSize, unit = speed.binSize_unit(record)
    df = pd.DataFrame(record["notes"])
    df_chords = chordDetection(df, 0.05)
...
def plot_chords_nps(record):
    """
    takes a record and plots the number of chords per second
    with notes per second on the same plot
    """
    df = pd.DataFrame(record["notes"])
    df_chords = chordDetection(df, 0.05)

    notes, unit, binSize = speed.binned_number_of_notes(record)
    ...
    # add legend
    plt.legend(["Chords/min", "Notes/sec"])
    plt.xlabel(unit)
    plt.ylabel("Value")
    plt.title("Chords vs Time")
